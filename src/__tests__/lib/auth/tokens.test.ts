...
mock.module('@/lib/providers/auth', () => {
  return {
    getAuthProvider: async (): Promise<{
      storeToken: () => Promise<void>;
      validateToken: () => Promise<boolean>;
      invalidateToken: () => Promise<void>;
      blacklistToken: (token: string) => Promise<void>;
      isBlacklisted: (token: string) => Promise<boolean>;
    }> => ({
      storeToken: async (): Promise<void> => {},
      validateToken: async (): Promise<boolean> => true,
      invalidateToken: async (): Promise<void> => {},
...
describe('Token Authentication', () => {
  ...
  describe('generateToken', () => {
    test('should generate a valid access token', async () => {
      ...
      const headerJson = Buffer.from(tokenParts[0] || '', 'base64').toString('utf-8');
    ...
    test('should throw error for invalid token type', async () => {
      const invalidPayload = { ...validPayload, type: 'invalid' } as unknown as TokenPayload;
      await expect(generateToken(invalidPayload)).rejects.toThrow('Non-standard token type');
    });
  ...
  describe('generateCsrfToken', () => {
    test('should generate a valid CSRF token', async () => {
      ...
      expect(verified.email).toBe('csrf@hyperauth.local');
    });
    ...
    test('should generate a CSRF token with correct structure', async () => {
      ...
      expect(payload).toHaveProperty('email', 'csrf@hyperauth.local');
  ...
  describe('verifyToken', () => {
    ...
    test('should throw error for blacklisted token', async () => {
      ...
      await expect(verifyToken(token)).rejects.toHaveProperty('code', JOSE_ERROR_CODES.REVOKED);
    });
    ...
    test('should throw error for expired token', async () => {
      ...
      await expect(validateTokenPayload(expiredPayload)).rejects.toThrow('Token has expired');
      await expect(validateTokenPayload(expiredPayload)).rejects.toHaveProperty('code', JOSE_ERROR_CODES.EXPIRED);
    });
  });
  ...
  describe('validateTokenPayload', () => {
    ...
    test('should reject payload with invalid token type', async () => {
      const invalidPayload = { ...validPayload, type: 'invalid' } as unknown as TokenPayload;
      await expect(validateTokenPayload(invalidPayload)).rejects.toThrow('Invalid token type');
    ...
    test('should reject payload with invalid role', async () => {
      const invalidPayload = { ...validPayload, role: 'superuser' } as unknown as TokenPayload;
      await expect(validateTokenPayload(invalidPayload)).rejects.toThrow('Invalid role');
      await expect(validateTokenPayload(invalidPayload)).rejects.toHaveProperty(
        'code',
        JOSE_ERROR_CODES.CLAIM_VALIDATION_FAILED
      );
    ...
    test('should reject payload with invalid email', async () => {
      ...
      await expect(validateTokenPayload(invalidPayload)).rejects.toThrow('Invalid email format');
      await expect(validateTokenPayload(invalidPayload)).rejects.toHaveProperty(
        'code',
        JOSE_ERROR_CODES.CLAIM_VALIDATION_FAILED
      );
    ...
    test('should allow csrf token with non-email format', async () => {
      ...
      expect(decoded.type).toBe('csrf');
      expect(decoded.email).toBe('csrf@hyperauth.local');
    });

    test('should reject payload with missing required claims', async () => {
      const incompletePayload = { ...validPayload };

      await expect(validateTokenPayload(incompletePayload)).rejects.toThrow('Missing required claims');
      await expect(validateTokenPayload(incompletePayload)).rejects.toHaveProperty(
        'code',
        JOSE_ERROR_CODES.CLAIM_VALIDATION_FAILED
      );
    });
    ...
    test('should reject payload with future issuance date', async () => {
      ...
      await expect(validateTokenPayload(futurePayload)).rejects.toThrow('Token not yet valid');
    ...
    test('should handle clock skew between servers gracefully', async () => {
      ...
      await expect(validateTokenPayload(payload)).rejects.toThrow('Token not yet valid');
    ...
    test.skip('should implement secure clock skew tolerance', async () => {
      ...
      await expect(validateTokenPayload(badPayload)).rejects.toThrow('Token not yet valid');
    });
  });
  ...
  describe('blacklistToken and resetBlacklist', () => {
    test('should blacklist a token and prevent its verification', async () => {
      ...
      const otherToken = await generateToken({ ...validPayload, email: 'other@example.com' });
    ...
    test('should correctly blacklist multiple tokens', async () => {
      const token1 = await generateToken({ ...validPayload, email: 'user1@example.com' });
      const token2 = await generateToken({ ...validPayload, email: 'user2@example.com' });
      const token3 = await generateToken({ ...validPayload, email: 'user3@example.com' });

      await blacklistToken(token1);
      await blacklistToken(token2);
  ...
  describe('Token types and roles validation', () => {
    ...
    test('should throw error for invalid role', async () => {
      ...
      const payload = {
        ...validPayload,
        role: invalidRole as unknown as UserRole,
        exp: Math.floor(Date.now() / 1000) + 3600,
        iat: Math.floor(Date.now() / 1000),
      };

      await expect(validateTokenPayload(payload)).rejects.toThrow('Invalid role');
      await expect(validateTokenPayload(payload)).rejects.toHaveProperty(
        'code',
        JOSE_ERROR_CODES.CLAIM_VALIDATION_FAILED
      );
  ...
  describe('Security edge cases', () => {
    test('should reject token with empty email', async () => {
      ...
      await expect(validateTokenPayload(payload)).rejects.toThrow('Invalid email format');
      await expect(validateTokenPayload(payload)).rejects.toHaveProperty(
        'code',
        JOSE_ERROR_CODES.CLAIM_VALIDATION_FAILED
      );
    ...
    test.skip('should prevent privilege escalation through token manipulation', async () => {
      ...
      await expect(createAdminToken()).rejects.toThrow('Unauthorized role escalation');
    });
    ...
    test('should match token expiration with constants definition', async () => {
      const accessToken = await generateToken({ ...validPayload, type: 'access' });
      ...
      const refreshToken = await generateToken({ ...validPayload, type: 'refresh' });
      ...
      const magicToken = await generateToken({ ...validPayload, type: 'magic' });
    ...
    test('should properly validate different token types with correct expiry times', async () => {
      ...
      for (const { type, expectedExpiry } of testCases) {
        const payload = { ...validPayload, type } as unknown as TokenPayload;
        const token = await generateToken(payload);
        const decoded = await verifyToken(token);
    ...
    test('should validate token structure for all token types', async () => {
      for (const type of TOKEN_TYPES) {
        const typePayload = { ...validPayload, type } as unknown as TokenPayload;
    ...
    test('should detect common JWT tampering techniques', async () => {
      ...
      headerObj.alg = 'none';
      const tamperedHeaderB64 = Buffer.from(JSON.stringify(headerObj)).toString('base64');

      const noneAlgToken = `${tamperedHeaderB64}.${payloadB64}.${signatureB64}`;
      await expect(verifyToken(noneAlgToken)).rejects.toThrow();

      headerObj.alg = 'HS512';
      const differentAlgHeaderB64 = Buffer.from(JSON.stringify(headerObj)).toString('base64');
