...
const categories = {
  ...
  rate: { icon: '⏱️', description: 'Rate limiting' },
...
export type LogCategory = 'auth' | 'token' | 'email' | 'http' | 'rate' | 'ui' | 'user' | 'db' | 'system' | 'api';
...
const MAX_LOG_RETENTION = MAX_RETENTION_DAYS * HOURS_PER_DAY * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * 1000;
...
function formatMessage(type: string, message: string, data?: Record<string, unknown>): string {
...
function storeLog(entry: LogEntry): void {
  ...
  if (logStorage.length > MAX_LOGS_IN_MEMORY) {
    logStorage.splice(0, Math.floor(MAX_LOGS_IN_MEMORY * STORAGE_CLEANUP_FACTOR));
  }
...
export const logHandler = {
  error: (category: LogCategory, message: string, data?: Record<string, unknown>, context?: Context): void =>
    log(LogLevel.ERROR, category, message, data, context),
  warn: (category: LogCategory, message: string, data?: Record<string, unknown>, context?: Context): void =>
    log(LogLevel.WARN, category, message, data, context),
  info: (category: LogCategory, message: string, data?: Record<string, unknown>, context?: Context): void =>
    log(LogLevel.INFO, category, message, data, context),
  debug: (category: LogCategory, message: string, data?: Record<string, unknown>, context?: Context): void =>
