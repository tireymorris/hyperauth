...
function handleError(c: Context, error: unknown): Response {
  ...
  const statusCode = validStatuses.includes(response.status as (typeof validStatuses)[number])
    ? response.status
    : HTTP_STATUS.INTERNAL_SERVER_ERROR;
  c.status(statusCode as 400 | 401 | 403 | 404 | 405 | 408 | 409 | 429 | 500 | 502 | 503 | 504);
  ...
  return c.redirect(`/?error=${encodeURIComponent(response.message || 'An error occurred')}&status=${response.status}`);
}
...
function createErrorResponse(error: unknown): ErrorResponse {
  ...
  if (error instanceof Error) {
    ...
    const status = statusMatch && statusMatch[1] ? parseInt(statusMatch[1], 10) : HTTP_STATUS.INTERNAL_SERVER_ERROR;

    return {
      status,
      message: statusMatch ? error.message.substring(ERROR_PREFIX_LENGTH).trim() : error.message,
      code: error.name,
